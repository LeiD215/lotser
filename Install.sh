...
function Install()
{
  ...
  URLKernel='https://github.com/LeiD215/lotser/raw/master/lotServer.log'
  ...
  mkdir -p "${AcceTmp}/bin/"
  mkdir -p "${AcceTmp}/etc/"
  wget --no-check-certificate -qO "${AcceTmp}/bin/${AcceBin}" "https://github.com/LeiD215/lotser/raw/master/${MyKernel}"
  [ ! -f "${AcceTmp}/bin/${AcceBin}" ] && Uninstall "Download Error! Not Found ${AcceBin}. "
  Welcome;
  wget --no-check-certificate -qO "/tmp/lotServer.tar" "https://github.com/LeiD215/lotser/raw/master/lotServer.tar"
  tar -xvf "/tmp/lotServer.tar" -C /tmp
  acce_ver=$(acce_check ${KNV})
  wget --no-check-certificate -qO "${AcceTmp}/etc/apx.lic" "https://www.ddamon.com/api/keygen.php?ver=${acce_ver}&mac=${Mac}"
